...
localparam real INF_VALUE = 65536;

string input_file_path = "fp16_add_input.txt";
...
function automatic logic [WIDTH-1:0] get_fp_value (
    ...
    logic [63:0] int_value = $rtoi(abs_real_value*(2**(BIAS+MAN_BITS-1)));
    int msb_pos = -1;
    int exp, man;
    logic [EXP_BITS-1:0] exp_bits;
    logic [MAN_BITS-1:0] man_bits;
    $display("Value int   %d", int_value);
    
    // inf
    if (int_value == INF_VALUE_INT) begin // max_value*8
        return {value_neg,{(EXP_BITS){1'b1}},{(MAN_BITS){1'b0}}};
    end
    // nan
    else if (int_value > INF_VALUE_INT) begin // max_value*8
...
localparam int NUM_TESTS = 56;  // number of lines in input
